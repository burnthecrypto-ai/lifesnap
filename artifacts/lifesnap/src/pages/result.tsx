...
export default function ResultPage() {
  ...
  const handleSave = () => {
    createSnapshot.mutate({
      data: {
        originalText: "Input text hidden for privacy", // Usually would store original text if needed
        mode,
        diaryEntry,
        snapshot,
  ...
  return (
    <div className="flex-1 flex flex-col relative h-full bg-background print:bg-white print:text-black">
      <div className="p-4 border-b border-border/40 flex items-center gap-3 sticky top-0 bg-background/90 backdrop-blur z-10 print:hidden">
        <Button variant="ghost" size="icon" onClick={() => setLocation("/")}>
          <ArrowLeft className="w-5 h-5" />
        </Button>
        <h2 className="font-semibold">Your LifeSnap</h2>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-[430px] mx-auto p-4 bg-background/80 backdrop-blur-xl border-t border-border/40 print:hidden z-20">
        <div className="flex gap-2">
          ...
          <Button 
            className="flex-[2] bg-gradient-to-r from-primary to-secondary text-white border-0" 
            onClick={handleSave} 
